...
export const AIMentorPane = ({ session, onSelectTab }) => {
  ...
  return (
    <div className="flex-1 bg-slate-950 rounded-xl border border-slate-800 flex flex-col overflow-hidden font-sans text-xs shadow-2xl">
      {/* Header Toolbar */}
      ...
      <div className="px-3.5 py-2 bg-slate-900/40 border-b border-slate-800/80">
        ...
        <div className="grid grid-cols-4 gap-1.5">
          {[
            ...
            <button
              ...
              disabled={loading}
              className={`p-1 rounded text-[10px] font-mono transition-all border text-center ${
                currentHintLevel === item.level
                  ? 'bg-amber-500/20 text-amber-300 border-amber-500/50 shadow-sm'
                  : item.level < currentHintLevel
                  ? 'bg-slate-900 text-slate-300 border-slate-700 hover:bg-slate-800'
                  : 'bg-slate-950/60 text-slate-500 border-slate-800/80 hover:bg-slate-900/60'
              }`}
            >
              {item.label}
            </button>
      ...
      <div className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-900/30 border-b border-slate-800 overflow-x-auto scrollbar-none">
        <button
          onClick={handleDiagnose}
          disabled={loading}
          className="px-2 py-1 rounded bg-slate-800/70 hover:bg-cyan-950/40 hover:text-cyan-300 border border-slate-700 text-[10px] font-mono text-slate-300 flex items-center gap-1 whitespace-nowrap transition-colors"
        >
          <Search className="w-3 h-3 text-cyan-400" /> Analyze Failure
        </button>
        <button
          onClick={() => handleGetHint()}
          disabled={loading}
          className="px-2 py-1 rounded bg-slate-800/70 hover:bg-amber-950/40 hover:text-amber-300 border border-slate-700 text-[10px] font-mono text-slate-300 flex items-center gap-1 whitespace-nowrap transition-colors"
        >
        ...
        <button
          ...
          disabled={loading}
          className="px-2 py-1 rounded bg-slate-800/70 hover:bg-indigo-950/40 hover:text-indigo-300 border border-slate-700 text-[10px] font-mono text-slate-300 flex items-center gap-1 whitespace-nowrap transition-colors"
        >
      ...
      {/* Message Stream */}
      <div className="flex-1 overflow-y-auto p-3.5 space-y-3 select-text bg-slate-950/70">
        {initialLoading ? (
              ...
              >
                ...
                >
                  ...
                  {!isUser && (msg.mode || msg.hintLevel) && (
                    <div className="flex items-center justify-between border-b border-slate-800/80 pb-1.5 mb-1.5 text-[10px] font-mono">
                  ...
                  {msg.observations && msg.observations.length > 0 && (
                    <div className="bg-slate-950/70 p-2 rounded-lg border border-slate-800 text-[11px] space-y-1 mt-2">
                  ...
                  {msg.nextSteps && msg.nextSteps.length > 0 && (
                    <div className="bg-cyan-950/20 border border-cyan-500/20 p-2 rounded-lg text-[11px] space-y-1 mt-2">
                      <span className="text-[10px] font-mono font-bold text-cyan-400 block uppercase flex items-center gap-1">
                            ...
                            <li key={sIdx} className="flex items-start justify-between gap-1">
                              ...
                              {isCommand && (
                                <button
                                  ...
                                  className="text-[9px] font-mono text-cyan-400 hover:text-cyan-300 shrink-0 p-0.5"
                  ...
                  {!isUser && (
                    <div className="flex items-center gap-2 pt-1 border-t border-slate-800/60 text-[10px] font-mono text-slate-400">
                      <button
                ...
                {isUser && (
                  <div className="w-6 h-6 rounded-lg bg-blue-600 text-white flex items-center justify-center shrink-0 mt-0.5 text-[10px] font-bold">
                    U
                  </div>
                )}
              </div>
      ...
      <div className="p-2.5 bg-slate-900/90 border-t border-slate-800 space-y-1.5">
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 bg-slate-950 border border-slate-800 focus:border-cyan-500/50 rounded-lg px-3 py-2 text-xs text-slate-100 placeholder-slate-500 outline-none transition-colors"
          />
          <Button
            variant="primary"
            size="sm"
